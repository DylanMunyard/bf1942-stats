...
public class PlayerRoundsMigrationService(HttpClient httpClient, string clickHouseUrl, ILogger<PlayerRoundsMigrationService> logger) : BaseClickHouseService(httpClient, clickHouseUrl)
{
        ...
        int delayMs = 5000)
    {
        ...
        try
        {
            logger.LogInformation("Starting player_rounds migration to add game column using server_online_counts JOIN");
            ...
            if (months.Count == 0)
            {
                logger.LogInformation("No data found in player_rounds; nothing to migrate.");
...
SETTINGS index_granularity = 8192";
        ...
        logger.LogInformation("Created player_rounds_v2 table with game column");
    }
    ...
    public async Task<bool> SwitchToNewTableAsync()
    {
        try
        {
            logger.LogInformation("Switching tables: player_rounds -> player_rounds_backup, player_rounds_v2 -> player_rounds");
    ...
    public async Task<bool> CleanupOldTableAsync()
    {
        try
        {
            logger.LogInformation("Dropping old backup table player_rounds_backup");
